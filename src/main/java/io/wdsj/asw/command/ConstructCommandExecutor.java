    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("status") && (sender.hasPermission("advancedsensitivewords.status") || sender instanceof ConsoleCommandSender)) {
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&', messagesManager.getProperty(PluginMessages.MESSAGE_ON_COMMAND_STATUS).replace("%NUM%", "&a" + messagesFilteredNum.get()).replace("%MODE%", checkProtocolLib() ? "&a快速模式" : "&c兼容模式").replace("%INIT%", isInitialized ? "&a已初始化" : "&c未初始化").replace("%MS%", getProcessAverage() >= 120 ? getProcessAverage() >= 300 ? "&c" + getProcessAverage() + "ms" : "&e" + getProcessAverage() + "ms" : "&a" + getProcessAverage() + "ms").replace("%VERSION%", AdvancedSensitiveWords.getInstance().getDescription().getVersion()).replace("%MC_VERSION%", getMinecraftVersion()).replace("%PLATFORM%", OsUtil.isWindows() ? "Windows" : (OsUtil.isMac() ? "Mac" : isUnix() ? "Linux" : "Unknown")).replace("%BIT%", is64() ? "64bit" : "32bit").replace("%JAVA_VERSION%", getJvmVersion()).replace("%JAVA_VENDOR%", getJvmVendor()).replace("%API_STATUS%", settingsManager.getProperty(PluginSettings.ENABLE_API) ? "&a启用" : "&c禁用")));
                return true;
            }
